...
type Card = { label: string; text: string; image?: string; layout: "bottom" | "top" | "center" };
...
function createCards(title: string, focus: string, previous: Card[]): Card[] {
  ...
  const bodyCards = sentences.slice(0, 6).map((text, index) => ({ label: `INSIGHT ${String(index + 1).padStart(2, "0")}`, text: text.length > 78 ? `${text.slice(0, 77)}…` : text, image: previous[index + 1]?.image, layout: (index % 3 === 0 ? "bottom" : index % 3 === 1 ? "top" : "center") as Card["layout"] }));
  return [{ label: "BOOK NOTE", text: title, image: previous[0]?.image, layout: "bottom" }, ...bodyCards];
}
export default function Home() {
  ...
  const [cards, setCards] = useState<Card[]>(() => createCards("결정이 어렵다면 지켜야 할 태도", focusSample, [])); const [active, setActive] = useState(0); const [accent, setAccent] = useState("#f05a3c"); const current = cards[active];
  ...
  async function saveCard(index = active) {
    ...
    const grad = ctx.createLinearGradient(0, 650, 0, 1350); grad.addColorStop(0, "transparent"); grad.addColorStop(1, "rgba(0,0,0,.72)"); ctx.fillStyle = grad; ctx.fillRect(0, 0, 1080, 1350);
    const lines = wrap(ctx, card.text, 820, "700 54px Arial"); const boxH = lines.length * 74 + 86; const y = card.layout === "top" ? 120 : card.layout === "center" ? (1350 - boxH) / 2 : 1350 - boxH - 110;
    ctx.fillStyle = "white"; ctx.fillRect(80, y, 920, boxH); ctx.strokeStyle = "#111"; ctx.lineWidth = 4; ctx.strokeRect(80, y, 920, boxH); ctx.fillStyle = "#111"; ctx.font = "700 54px Arial"; lines.forEach((line, i) => ctx.fillText(line, 125, y + 76 + i * 74));
    ctx.fillStyle = accent; ctx.fillRect(80, 62, 105, 10); ctx.fillStyle = "white"; ctx.font = "700 24px Arial"; ctx.fillText(card.label, 80, 112); const link = document.createElement("a"); link.download = `cardletter-${index + 1}.png`; link.href = canvas.toDataURL("image/png"); link.click();
  }
  ...
  return <main className="app-shell"><header><div className="brand"><span>CL</span><b>CARDLETTER</b><small>블로그에서 골라 만드는 카드뉴스</small></div><button className="save-all" onClick={saveAll}>전체 PNG 저장</button></header>
    ...
    <section className="workspace"><aside className="input-column"><div className="section-head"><span>01</span><div><h2>블로그 글</h2><p>전체 글을 붙여 넣고, 카드로 만들 부분만 골라주세요.</p></div></div>
      ...
      <section className="canvas-column"><div className="section-head"><span>02</span><div><h2>사진과 문구</h2><p>사진을 순서대로 넣고, 각 장의 문구를 다듬으세요.</p></div><label className="upload">사진 추가<input type="file" accept="image/*" multiple onChange={addPhotos}/></label></div>
        <div className="editor-grid"><div className="stage"><article className={`photo-card ${current.layout}`} style={current.image ? { backgroundImage: `url(${current.image})` } : undefined}>{!current.image && <div className="empty-photo"><b>사진을 추가하세요</b><span>권장 1080 × 1350</span></div>}<div className="shade"/><div className="card-mark" style={{borderColor: accent}}>{current.label}</div><div className="text-box">{current.text}</div><div className="count">{active + 1} / {cards.length}</div></article></div>
          <div className="controls"><label>이 장의 문구<textarea value={current.text} onChange={(e) => updateCard({ text: e.target.value })}/></label><label>작은 라벨<input value={current.label} onChange={(e) => updateCard({ label: e.target.value })}/></label>
            <div className="control-row"><div><span>문구 위치</span><div className="segmented">{(["top","center","bottom"] as Card["layout"][]).map((v) => <button key={v} className={current.layout === v ? "on" : ""} onClick={() => updateCard({layout:v})}>{v === "top" ? "위" : v === "center" ? "가운데" : "아래"}</button>)}</div></div><label className="color">포인트<input type="color" value={accent} onChange={(e) => setAccent(e.target.value)}/></label></div>
