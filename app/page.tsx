...
function CardCanvas({ card, index }: { card: Card; index: number }) { const ref = useRef<HTMLCanvasElement>(null); useEffect(() => { if (ref.current) renderCard(ref.current, card, index); }, [card, index]); return <canvas ref={ref} width={1080} height={1350} style={{width:"min(100%, 430px)",height:"auto",display:"block",boxShadow:"0 18px 38px #28251d32"}} aria-label={`${index + 1}번째 카드 미리보기`}/>; }
async function renderCard(canvas: HTMLCanvasElement, card: Card, index: number) {
  ...
  if (card.image) { const img = new Image(); img.src = card.image; try { await img.decode(); const scale = Math.max(1080 / img.width, 1350 / img.height); const w = img.width * scale, h = img.height * scale; ctx.drawImage(img, (1080 - w) / 2, (1350 - h) / 2, w, h); } catch {} }
  ...
  if (index === 0) { const grad = ctx.createLinearGradient(0, 590, 0, 1350); grad.addColorStop(0, "transparent"); grad.addColorStop(1, "rgba(0,0,0,.86)"); ctx.fillStyle = grad; ctx.fillRect(0, 0, 1080, 1350); const lines = wrap(ctx, card.text, 790, "700 66px Arial"); ctx.fillStyle = "white"; ctx.font = "700 66px Arial"; const firstBaseline = 1090 - (lines.length - 1) * 43; lines.forEach((line, i) => ctx.fillText(line, 145, firstBaseline + i * 86)); return; }
  const font = "700 38px Arial"; const lineHeight = 54; const padX = 38; const padY = 30; const lines = wrap(ctx, card.text, 760, font); ctx.font = font; const widest = Math.max(...lines.map((line) => ctx.measureText(line).width)); const boxW = Math.min(836, Math.max(360, Math.ceil(widest + padX * 2))); const boxH = lines.length * lineHeight + padY * 2; const x = card.layout === "center" ? Math.round((1080 - boxW) / 2) : 54; const y = card.layout === "top" ? 92 : card.layout === "center" ? Math.round((1350 - boxH) / 2) : 1350 - boxH - 92; ctx.fillStyle = "white"; ctx.fillRect(x, y, boxW, boxH); ctx.strokeStyle = "#111"; ctx.lineWidth = 3; ctx.strokeRect(x, y, boxW, boxH); ctx.fillStyle = "#111"; ctx.textBaseline = "middle"; lines.forEach((line, i) => ctx.fillText(line, x + padX, y + padY + lineHeight / 2 + i * lineHeight)); ctx.textBaseline = "alphabetic";
}
